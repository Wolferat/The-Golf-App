(() => {
  ...
  const activeKey =
    page === 'game' ? 'game' : page === 'players' ? 'players' : ['settings','company','listings','listing-edit'].includes(page) ? 'account' : 'home';
  ...
  const icons = {
    ...
    account: '<svg class="nav-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.5"/><path d="M4 21v-2a8 8 0 0 1 16 0v2"/></svg>',
    players:
