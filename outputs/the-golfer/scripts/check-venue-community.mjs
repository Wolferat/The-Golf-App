...
assert(home.includes('Make time<br>for golf.'), 'Home must open into the Explore dashboard');
assert(home.includes('Find your next round'), 'Discovery board must show its player-facing heading');
...
assert(appNav.includes('Explore') && appNav.includes('My Game') && appNav.includes('Golf Crew'), 'Shared mobile nav must use consistent labels');
...
assert(home.includes('id="previewSignupCta"') && home.includes("$('#previewSignupCta').onclick=openModal"), 'Signed-out home must offer account creation');
...
assert(home.includes('browseAction') && home.includes('Explore local golf'), 'Signed-out home must use a browse action instead of faux search');
