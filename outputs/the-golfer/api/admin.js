...
export default async function handler(req,res) {
  ...
  if(req.method==='GET'){const r=await fetch(base+'?status=eq.pending&select=id,title,kind,city,source_name,source_url,created_at&order=created_at.desc',{headers});return res.status(r.status).json({listings:await r.json()})}
