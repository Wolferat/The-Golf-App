...
export default async function handler(req,res){
  ...
  if(req.method==='GET'){const r=await fetch(url+'&select=company_name,support_email,launch_boundary_name,review_mode',{headers});return res.status(r.status).json({settings:(await r.json())[0]||null})}
  if(req.method!=='PUT')return res.status(405).json({error:'Method not allowed.'});
  const {company_name,support_email,launch_boundary_name,review_mode}=req.body||{};
  const r=await fetch(url,{method:'PATCH',headers:{...headers,'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify({company_name,support_email,launch_boundary_name,review_mode,updated_at:new Date().toISOString()})});
